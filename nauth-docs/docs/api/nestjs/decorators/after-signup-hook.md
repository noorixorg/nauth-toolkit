---
title: "@AfterSignupHook()"
description: Decorator for automatic after-signup hook registration in NestJS
sidebar_position: 5
keywords: [decorator, hooks, lifecycle, aftersignup, notifications]
...
Class decorator that automatically registers a provider as an after-signup hook. After-signup hooks execute after successful user creation for notifications, integrations, and analytics. Non-blocking - errors are logged but don't affect signup.

:::tip Import from NestJS Package
```typescript
import { AfterSignupHook } from '@nauth-toolkit/nestjs';
```
:::

## Overview

The `@AfterSignupHook()` decorator enables automatic hook registration without manual setup. Classes decorated with this decorator are discovered at module initialization and registered with the [`HookRegistryService`](/docs/api/core/services/hook-registry-service).
...
**When After-Signup Hooks Execute:**
...
@Injectable()
@AfterSignupHook({ priority: 1 })  // Executes first
export class WelcomeEmailHook implements IAfterSignupHookProvider {
  async execute(user, metadata) {
    // Send welcome email
  }
...
@Injectable()
@AfterSignupHook({ priority: 2 })  // Executes second
...
@Injectable()
@AfterSignupHook()
export class UserProvisioningHook implements IAfterSignupHookProvider {
...
After-signup hooks are non-blocking. All errors are caught and logged automatically:

```typescript
@Injectable()
@AfterSignupHook()
export class NotificationHook implements IAfterSignupHookProvider {
  async execute(user, metadata) {
