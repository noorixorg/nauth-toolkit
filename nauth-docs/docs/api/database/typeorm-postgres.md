...
| `computeAdvisoryLockKey` | Derive the lock identifier from a migrations table name. |
...
Running several instances that boot at the same time — ECS tasks, Kubernetes pods, a rolling deploy — is safe with no configuration. Each run is serialized behind a PostgreSQL session-scoped **advisory lock** (`pg_try_advisory_lock`) derived from the migrations table name:
...
- The lock lives on a dedicated connection, so PostgreSQL drops it automatically if an instance crashes — a dead task can never wedge a deploy.
