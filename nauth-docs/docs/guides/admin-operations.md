...
Sends a reset code via email or SMS. The user completes the reset via [`POST /auth/reset-password/confirm`](#confirm-password-reset) below — **not** `/auth/forgot-password/confirm`, which is the self-service flow and validates a different code.
...
When `baseUrl` is set, the link also carries an `email` query param (e.g. `?code=123456&email=user%40example.com`) so the consumer app's reset page can prepopulate the identifier field instead of asking the user to retype it — the admin already has the address on hand, so this adds no exposure the self-service flow doesn't already have. The self-service `forgot-password` link (`AuthService.forgotPassword`) does not include `email`.

### Confirm password reset

Public endpoint — the user calls this directly with the code from the email/SMS, no admin auth involved. It only accepts codes generated by [Initiate password reset](#initiate-password-reset) above; a self-service `forgot-password` code is rejected here, and vice versa.
...
| --- | --- | --- |
| `identifier` | string | Email, username, or phone — matches the target user (required) |
| `code` | string | 6–10 digit code from the reset email/SMS (required) |
| `newPassword` | string | 8–128 chars (required) |

:::warning Wrong-endpoint failures look identical
Posting this code to `/auth/forgot-password/confirm` instead returns the same `PASSWORD_RESET_CODE_INVALID` error as a genuinely wrong code — there's no distinct error for "right code, wrong endpoint." If a frontend page handles both admin-sent and self-service reset links, pick the endpoint from something in the link itself — the admin link's `email` query param is one such marker (the self-service link never has one), or append your own to the admin `baseUrl`.
:::
...
The user completes the reset with `client.resetPasswordWithCode()` — a top-level method, not under `admin.*`, since the user calls it directly and it requires no admin auth:

```typescript
// On the page the reset link/email points to:
await auth.resetPasswordWithCode('user@example.com', '123456', 'NewSecurePass123!');
```

:::warning
`resetPasswordWithCode()` only completes an **admin-initiated** reset (it posts to `/auth/reset-password/confirm`). A self-service "forgot password" code — from `auth.forgotPassword()` — is completed with `auth.confirmForgotPassword()` instead; the two codes are not interchangeable. See [Confirm password reset](#confirm-password-reset) above.
:::
