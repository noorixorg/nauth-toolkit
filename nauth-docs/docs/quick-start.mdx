...
async function bootstrap() {
  ...
  fastify.post(
    ...
    withNAuthContext(async (req) => {
      const result = await nauth.authService.signup(req.body as any);
      return result;
    }),
  );
...
| **Route protection** | `@UseGuards(AuthGuard)` | `nauth.helpers.requireAuth()` | `preHandler` + `withNAuthContext` |
...
| **Context propagation** | Automatic | Automatic | Requires `withNAuthContext` wrapper |
