...
A complete working example is available at [github.com/noorixorg/nauth/tree/main/nestjs](https://github.com/noorixorg/nauth/tree/main/nestjs) — NestJS + TypeORM + PostgreSQL with lifecycle hooks, custom email templates, Google OAuth, and TOTP MFA preconfigured.
:::
...
@Module({
  imports: [
    TypeOrmModule.forRoot({
      ...
      synchronize: process.env.NODE_ENV === 'development', // ⚠️ See warning below
    }),
...
`synchronize: true` auto-creates database tables from entities --- convenient for development but **must be `false` in production**. Use TypeORM migrations instead: `typeorm migration:generate` and `typeorm migration:run`.
:::
...
- [NestJS Sample App](https://github.com/noorixorg/nauth/tree/main/nestjs) — Full working example with hooks, email templates, Google OAuth, and TOTP MFA
