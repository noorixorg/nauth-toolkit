...
async function bootstrap(): Promise<void> {
  ...
  app.enableCors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'x-csrf-token', 'x-device-token'],
  });
...
Configure the provider in your auth config — see the [Social Login](/docs/features/social-login) guide for complete setup.
...
Configure MFA settings in your auth config — see the [MFA](/docs/features/mfa) guide for complete setup.

:::tip Complete Route Examples
This quick start covers the minimal setup. For complete route implementations including password reset, MFA verification, social login callbacks, and challenge handling, see the [Authentication Routes](/docs/features/routes) guide.
:::
...
- [Token Delivery](/docs/features/token-delivery) — Switch between JSON (Bearer) and cookie-based token delivery
