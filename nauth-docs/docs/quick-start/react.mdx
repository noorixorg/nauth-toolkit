...
sidebar_position: 3
title: React
description: 'Connect React to nauth-toolkit with @nauth-toolkit/client: NAuthClient config, auto-refresh HttpAdapter, AuthContext and useAuth, ProtectedRoute, login/signup/dashboard pages, OAuth callback'
...
The sample app adds `ChallengePage.tsx` and `MfaSetupPage.tsx` for verification and MFA flows — see [Challenge Handling](#challenge-handling) below.
...
If you enable email or phone verification (`verificationMethod: 'email'`) or MFA, login and signup return a `challengeName` instead of tokens, and the pages above navigate to `/auth/challenge`. Copy [`ChallengePage.tsx`](https://github.com/noorixorg/nauth-toolkit/blob/main/examples/starter-react/src/pages/ChallengePage.tsx) and [`MfaSetupPage.tsx`](https://github.com/noorixorg/nauth-toolkit/blob/main/examples/starter-react/src/pages/MfaSetupPage.tsx) from the sample app and add their routes:
...
See the [Challenge System](/docs/concepts/challenge-system) guide for how the flow works.
