...
:::note Token Delivery
This guide uses `tokenDelivery: 'cookies'` with CSRF protection — the recommended setup for web apps. To use Bearer tokens instead, set `tokenDelivery: 'json'` and remove the `csrf` block (no CSRF config needed in JSON mode). See [Token Delivery](/docs/concepts/token-management) for details.
:::
...
- [Angular Sample App](https://github.com/noorixorg/nauth/tree/main/angular) — Full working example paired with the NestJS backend
