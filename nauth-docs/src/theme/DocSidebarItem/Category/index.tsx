 ...
 *
 * Swizzled (ejected) to support customProps.icon for sidebar category icons.
 * Minimal change: CategoryLinkLabel accepts an optional icon prop.
 */
...
// Modified to support an optional icon from customProps.icon
function CategoryLinkLabel({label, icon}: {label: string; icon?: string}) {
  return (
    <span title={label} className={styles.categoryLinkLabel}>
      {icon && <i className={`${icon} sidebar-nav-icon`} aria-hidden="true" />}
      {label}
    </span>
  );
}
