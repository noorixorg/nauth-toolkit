 ...
 * config settings end-to-end against demo-nestjs.
 *
 * demo-nestjs is configured with:
...
// Expected TTLs from demo-nestjs config/auth.config.ts hybridPolicy.
...
test.describe('Hybrid Per-Delivery Refresh TTL', () => {
  ...
  test('refresh TTL matches hybrid policy per delivery, and refresh expires at that boundary', async ({
    ...
    // In hybrid mode on demo-nestjs, /auth/respond-challenge has no
