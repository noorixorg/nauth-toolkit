 ...
 * import { createPinoLoggerAdapter } from './utils/pino-logger.adapter';
 *
 * const logger = pino({
 *   transport: {
 *     target: 'pino-pretty',
 ...
 * });
 *
 * const adapter = createPinoLoggerAdapter(logger);
 * ```
 */
 ...
 * });
 *
 * const adapter = createPinoLoggerAdapter(pinoLogger);
 * ```
 */
