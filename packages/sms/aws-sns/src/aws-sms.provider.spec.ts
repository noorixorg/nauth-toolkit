...
describe('AWSSMSProvider', () => {
  ...
  describe('constructor', () => {
    ...
    it('should accept a logger via setLogger', () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('AWS SMS Provider logger attached'));
    });
    ...
    it('should throw if region is missing', () => {
      ...
      expect(() => new AWSSMSProvider(config)).toThrow('AWS SMS Provider: region and originationNumber are required');
    });
  ...
  describe('sendOTP - SNS mode', () => {
    ...
    it('should handle SNS API errors', async () => {
      ...
      await expect(provider.sendOTP('+19876543210', '123456')).rejects.toThrow('AWS SMS delivery failed');
    });
  });
