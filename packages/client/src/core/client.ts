...
import { detectLocale, detectTimezone, withDetectedPreferences } from './environment';
...
export class NAuthClient {
  ...
  async signup(payload: SignupRequest): Promise<AuthResponse> {
    payload = withDetectedPreferences(payload, this.config.autoDetectLocale);
  ...
  /**
   * Complete an admin-initiated password reset with a verification code.
   *
   * NOTE:
   * - Admin-initiated only. A self-service "forgot password" code (from {@link forgotPassword}) is NOT
   *   accepted here — use {@link confirmForgotPassword} for that flow instead. The two codes are validated
   *   against different token types server-side and are not interchangeable, even though the emailed link
   *   format is identical between the two flows (`...?code=123456`, no distinguishing marker).
   *
   * WHY: Posts to the admin-reset confirm endpoint (`confirmAdminResetPassword`), which only recognizes
   * codes generated by an admin's `adminResetPassword` call.
   *
  ...
  async loginWithSocial(provider: SocialProvider, options?: SocialLoginOptions): Promise<void> {
    ...
    if (hasWindow()) {
      ...
      // Seed the new account's preferences if this flow ends up creating one. The browser
      // is not part of the provider's callback request, so the backend parks these in the
      // OAuth state and reads them back at callback time.
      if (this.config.autoDetectLocale) {
        const timezone = detectTimezone();
        const locale = detectLocale();
        if (timezone) startUrl.searchParams.set('timezone', timezone);
        if (locale) startUrl.searchParams.set('locale', locale);
      }
