...
/**
 * Switch API keys from lookupId-based lookup to direct keyHash lookup (MySQL).
 *
...
export class ApiKeysHashLookup1769300000001 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('DROP INDEX `IDX_nauth_api_keys_lookupId` ON `nauth_api_keys`');
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('DROP INDEX `IDX_nauth_api_keys_keyHash` ON `nauth_api_keys`');
    await queryRunner.query('ALTER TABLE `nauth_api_keys` ADD `lookupId` varchar(64) NULL');
  }
}
