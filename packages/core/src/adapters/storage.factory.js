...
function createDatabaseStorageAdapter() {
    ...
    var DatabaseStorageAdapter = require('@nauth-toolkit/storage-database').DatabaseStorageAdapter;
 ...
 * ```
 */
function createRedisStorageAdapter(url) {
    if (url === void 0) { url = 'redis://localhost:6379'; }
    // Lazy import to avoid bundling if not used
    var RedisStorageAdapter = require('@nauth-toolkit/storage-redis').RedisStorageAdapter;
    var createClient = require('redis').createClient;
    var redisClient = createClient({ url: url });
...
function createRedisClusterAdapter(nodes) {
    // Lazy import to avoid bundling if not used
    var RedisStorageAdapter = require('@nauth-toolkit/storage-redis').RedisStorageAdapter;
    var createCluster = require('redis').createCluster;
    var clusterClient = createCluster({
        rootNodes: nodes,
    });
    // Don't connect here - let adapter.initialize() handle connection
    // This ensures proper error handling and allows initialize() to wait for connection
